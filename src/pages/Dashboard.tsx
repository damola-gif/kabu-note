...
const StatCard = ({ title, value, valueClassName, description, children }: { title: string, value: React.ReactNode, valueClassName?: string, description?: string, children?: React.ReactNode }) => (
  <div className="bg-card shadow rounded-lg p-4 flex flex-col">
    <span className="text-lg font-bold text-card-foreground">{title}</span>
    <span className={cn("text-2xl font-mono", valueClassName)}>{value}</span>
  ...
  <>
    ...
    <div className="mb-6">
      <div className="overflow-x-auto flex gap-3">
        <Skeleton className="h-20 min-w-[150px] rounded-lg" />
        <Skeleton className="h-20 min-w-[150px] rounded-lg" />
        <Skeleton className="h-20 min-w-[150px] rounded-lg" />
      </div>
    </div>
...
export default function Dashboard() {
  ...
  return (
    <>
        ...
        <div className="overflow-x-auto flex gap-3 pb-2">
          {stats.openTrades.length > 0 ? (
            stats.openTrades.map((trade) => (
              <div key={trade.id} className="bg-card shadow rounded-lg p-3 min-w-[180px] border-l-4"
                style={{ borderLeftColor: trade.side === 'long' ? '#22c55e' : '#ef4444' }}>
                <div className="font-bold">{trade.symbol}</div>
                <div className="text-sm capitalize">{trade.side}</div>
      ...
      <div className="bg-card rounded-lg shadow p-4">
        <h3 className="text-lg font-semibold mb-2">Recent Closed Trades</h3>
            ...
            stats.closedTrades.map((trade) => (
              <div key={trade.id} className="py-2 flex justify-between items-center">
                <div>
                  ...
                  <span className="ml-2 text-sm text-muted-foreground capitalize">{trade.side}</span>
                </div>
                <div className="text-right">
                  <Badge variant={trade.pnl && trade.pnl > 0 ? "default" : "destructive"}
                    className={cn(trade.pnl && trade.pnl > 0 && "bg-green-600/20 text-green-700 border-green-600/30",
